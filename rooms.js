...
class Room {
	...
	isMuted(user) {
		if (!user) return;
		if (this.muteQueue) {
            for (let entry of this.muteQueue) {
...
class ChatRoom extends Room {
	...
	getIntroMessage(user) {
		...
		if (this.modchat) {
			message += `${message ? '<br />' : '\n|raw|<div class="infobox">'}<div class="broadcast-red">Must be rank ${this.modchat} or higher to talk right now.</div>`;
		}
		if (this.slowchat && user.can('mute', null, this)) {
			message += `${message ? '<br />' : '\n|raw|<div class="infobox">'}<div class="broadcast-red">Messages must have at least ${this.slowchat} seconds between them.</div>`;
		}
...
Rooms.createBattle = (roomid, format, p1, p2, options) => {
	if (roomid && roomid.id) return roomid;
...
Rooms.createChatRoom = (roomid, title, data) => {
